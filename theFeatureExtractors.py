...
class myExtractor(FeatureExtractor):
    ...
    # Returns if any ghosts are current weak
    def ghostAreWeak(self, ghostStates):
        for gState in ghostStates:
            if gState.scaredTimer > 0:
                return True
        return False

    # Returns a list of all ghosts that are currently scared
    def getWeakGhostStates(self, ghostStates):
        return list(filter(lambda ghostState: ghostState.scaredTimer > 0, ghostStates))

    # Returns a list of all ghost that are not scared
    def getStrongGhostStates(self, ghostStates):
        return list(filter(lambda ghostState: ghostState.scaredTimer == 0, ghostStates))

    # Returns True if a non-scared ghost is 1 move away
    ...
    def getFeatures(self, state, action):
        ...
        features['#-of-strong-ghost-1-step-away'] = sum(
            (next_x, next_y) in Actions.getLegalNeighbors(ghostState.getPosition(), walls) for ghostState in self.getStrongGhostStates(ghostStates))
        ...
        if not self.ghostAreWeak(ghostStates) and \
